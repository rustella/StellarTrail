...
async fn unauthenticated_requests_are_rate_limited_by_ip() {
    ...
    let (first_status, _, _) =
        send_empty(&app.router, "GET", "/api/v1/meta", None, [203, 0, 113, 10]).await;
...
async fn ip_limit_is_shared_across_routes_and_fallback() {
    ...
    let (meta_status, _, _) =
        send_empty(&app.router, "GET", "/api/v1/meta", None, [203, 0, 113, 11]).await;
    let (missing_status, body, _) = send_empty(
        &app.router,
        "GET",
        "/api/v1/missing",
        None,
...
async fn different_ips_have_separate_buckets() {
    ...
    let (first_status, _, _) =
        send_empty(&app.router, "GET", "/api/v1/meta", None, [203, 0, 113, 12]).await;
    let (second_status, _, _) =
        send_empty(&app.router, "GET", "/api/v1/meta", None, [203, 0, 113, 12]).await;
    let (other_ip_status, _, _) =
        send_empty(&app.router, "GET", "/api/v1/meta", None, [198, 51, 100, 12]).await;
...
async fn trusted_proxy_headers_choose_forwarded_client_ip() {
    ...
    let (forwarded_limited_status, _, _) =
        send_empty_with_xff(&app.router, "/api/v1/meta", [172, 16, 0, 3], "203.0.113.13").await;
    let (other_forwarded_status, _, _) =
        send_empty_with_xff(&app.router, "/api/v1/meta", [172, 16, 0, 3], "203.0.113.14").await;
...
async fn untrusted_proxy_headers_are_ignored() {
    ...
    .await;
    let (direct_ip_limited_status, _, _) = send_empty_with_xff(
        &app.router,
        "/api/v1/meta",
        [198, 51, 100, 20],
        "203.0.113.21",
    ...
    .await;
    let (same_forwarded_different_direct_status, _, _) = send_empty_with_xff(
        &app.router,
        "/api/v1/meta",
        [198, 51, 100, 21],
        "203.0.113.20",
...
async fn disabling_global_rate_limit_preserves_original_route_behavior() {
    ...
    let (first_status, _, _) =
        send_empty(&app.router, "GET", "/api/v1/meta", None, [203, 0, 113, 50]).await;
    let (second_status, _, _) =
        send_empty(&app.router, "GET", "/api/v1/meta", None, [203, 0, 113, 50]).await;
