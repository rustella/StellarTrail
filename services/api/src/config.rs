...
cors:
  allowed_origins:
    - https://app.stellartail.cn
    - https://www.stellartail.cn
...
"#,
        ...
        assert_eq!(
            ...
            vec![
                "https://app.stellartail.cn".to_owned(),
                "https://www.stellartail.cn".to_owned(),
            ]
        );
        assert!(config.cors.allow_credentials);
        assert!(config.mail.enabled);
        assert_eq!(config.mail.smtp_host, "mx1.stellartrail.cn");
        assert_eq!(config.mail.smtp_port, 465);
        assert_eq!(config.mail.smtp_tls, MailSmtpTls::Implicit);
        assert_eq!(config.mail.smtp_username, "noreply@stellartrail.cn");
        assert_eq!(config.mail.smtp_password, "example-mail-password");
        assert_eq!(config.mail.from, "寻径星野 <noreply@stellartrail.cn>");
...
"#,
        ...
        unsafe {
            ...
            env::set_var(
                ...
                "https://app.stellartail.cn, https://stellartail.cn, https://www.stellartail.cn",
            );
    ...
    fn from_env_rejects_cors_origins_with_paths() {
        ...
        unsafe {
            clear_env(CONFIG_KEYS);
            env::set_var("DATABASE_URL", "sqlite://stellartrail.db");
            env::set_var("CORS_ALLOWED_ORIGINS", "https://app.stellartail.cn/path");
        }
    ...
    fn from_env_reads_mail_config_from_environment() {
        ...
        unsafe {
            ...
            env::set_var("MAIL_SMTP_HOST", " mx1.stellartrail.cn ");
            env::set_var("MAIL_SMTP_PORT", "465");
            env::set_var("MAIL_SMTP_TLS", "implicit");
            env::set_var("MAIL_SMTP_USERNAME", " noreply@stellartrail.cn ");
            env::set_var("MAIL_SMTP_PASSWORD", " example-mail-password ");
            env::set_var("MAIL_FROM", " 寻径星野 <noreply@stellartrail.cn> ");
