...
- `infra/production/api/docker-compose.yml`：后端 API，域名 `api.stellartail.cn`，根目录 `config.yaml` 挂载到容器 `/app/config.yaml:ro`。
- `infra/production/api-deps/docker-compose.yml`：API 依赖组件 PostgreSQL、Redis、MinIO；`assets.stellartail.cn` 只通过 Traefik 指向 MinIO API，MinIO console 不直接公网暴露。
