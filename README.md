...
绳结媒体通过 MinIO/S3-compatible 对象存储上传，公开读接口只返回数据库中的媒体 URL，不再从 `/assets/*` 拼接绳结媒体路径。服务端只维护一组 `minio` 连接配置，反馈图与绳结媒体分别通过 `object_storage.bucket` 和 `knots_media_storage.bucket` 配置业务 bucket。管理员权限存储在数据库 `admin_roles` 表：已有且未删除的 `stellarisw` 用户会在迁移时被 seed 为 `super_admin`，`super_admin` 可通过 `/api/admin/admins` 授予或移除普通 `admin`。`admin` 与 `super_admin` 都可调用 Knots 媒体上传、Gear Atlas 审核、反馈查看和 `GET /api/admin/api-usage`。统计使用异步上报，只保存 matched route 模板和聚合计数，不记录 query、请求体、Authorization、token、Cookie、IP 或 User-Agent。
