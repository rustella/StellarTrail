  ...
  /******/ [
      ...
      Component({
        ...
        ready: function ready() {
          if (!this.data.echarts) {
            console.warn(
              "The ec-canvas component requires the echarts instance.",
            );
            return;
          }
          ...
          if (!this.data.ec) {
            console.warn(
              'The ec-canvas component requires the ec option, for example: <ec-canvas id="mychart-dom-bar" ' +
        ...
        methods: {
          init: function init(callback) {
            ...
            if (forceUseOldCanvas && canUseNewCanvas) {
              console.warn(
                "forceUseOldCanvas is enabled; disable it to use the newer canvas when available.",
              );
            }

            if (isUseNewCanvas) {
              // Base library versions newer than 2.9.0 can use <canvas type="2d"></canvas>.
              this.initByNewWay(callback);
            } else {
              var isValid = compareVersion(version, "1.9.91") >= 0;
              if (!isValid) {
                console.error(
                  "The WeChat base library version is too old; it must be >= 1.9.91. " +
                    "See https://github.com/ecomfe/echarts-for-weixin#%E5%BE%AE%E4%BF%A1%E7%89%88%E6%9C%AC%E8%A6%81%E6%B1%82",
                );
                return;
              } else {
                console.warn(
                  "Use WeChat base library >= 2.9.0 for better chart rendering performance.",
                );
          ...
          initByOldWay: function initByOldWay(callback) {
            ...
            // 1.9.91 <= version < 2.9.0: use the legacy initialization path.
            ...
            // const canvasDpr = wx.getSystemInfoSync().pixelRatio // The legacy canvas cannot receive dpr.
            var canvasDpr = 1;
            ...
            query
              ...
              .boundingClientRect(function (res) {
                if (typeof callback === "function") {
                  ...
                  _this.data.ec &&
                  typeof _this.data.ec.onInit === "function"
                ) {
                  _this.chart = _this.data.ec.onInit(
                    canvas,
                    res.width,
                    res.height,
                    canvasDpr,
                  ...
                  _this.triggerEvent("init", {
                    ...
                    width: res.width,
                    height: res.height,
                    canvasDpr: canvasDpr, // Include dpr for external echarts.init calls.
                  });
                }
              })
              .exec();
          ...
          initByNewWay: function initByNewWay(callback) {
            ...
            // version >= 2.9.0: use the newer initialization path.
            var query = wx.createSelectorQuery().in(this);
            query
              .select(".ec-canvas")
              ...
              .exec(function (res) {
                ...
                var canvas = new _wxCanvas2.default(
                  ctx,
                  _this2.data.canvasId,
                  true,
                  canvasNode,
                );
                ...
                if (typeof callback === "function") {
                  ...
                  _this2.data.ec &&
                  typeof _this2.data.ec.onInit === "function"
                ) {
                  _this2.chart = _this2.data.ec.onInit(
                    canvas,
                    canvasWidth,
                    canvasHeight,
                    canvasDpr,
    ...
    /***/ function (module, exports, __webpack_require__) {
      ...
      var WxCanvas = (function () {
        ...
        WxCanvas.prototype.removeEventListener =
            ...
            Object.defineProperty(ctx, style, {
              set: function set(value) {
                if (
                  ...
                  ctx["set" + style.charAt(0).toUpperCase() + style.slice(1)](
                    value,
                  );
                }
              },
