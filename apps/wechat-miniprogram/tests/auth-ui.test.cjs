...
${ts}`;
  ...
  assert.match(wxml, /bindblur="saveWechatNickname"/);
  ...
  assert.match(pageSource, /wx as WechatProfileApi/);
  assert.match(pageSource, /getUserProfile/);
  assert.match(pageSource, /loginWithWechat\(\{ nickname \}\)/);
  ...
  assert.match(pageSource, /normalizeWechatNickname/);
