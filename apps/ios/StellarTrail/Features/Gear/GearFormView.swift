...
struct GearFormView: View {
    ...
    init(environment: AppEnvironment, item: GearItem? = nil) {
    ...
    var body: some View {
        Form {
            Section("基础信息") {
                ...
                TextField("材质", text: optionalText($viewModel.draft.material))
                TextField("描述", text: optionalText($viewModel.draft.description), axis: .vertical)
            }
            ...
            Section("规格购买") {
                ...
                TextField("保暖指标", text: optionalText($viewModel.draft.warmthIndex))
                TextField("防水指标", text: optionalText($viewModel.draft.waterproofIndex))
                TextField("购买日期", text: optionalText($viewModel.draft.purchaseDate))
                TextField("购买地点", text: optionalText($viewModel.draft.purchaseLocation))
                TextField("保修到期", text: optionalText($viewModel.draft.expiryOrWarrantyDate))
            }
            ...
            Section {
                Button(viewModel.loading ? "保存中…" : "保存装备") {
