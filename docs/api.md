...
GET /api/v1/content-pages/profile_about?client_key=android&locale=zh-CN
GET /api/v1/client-versions?client_key=android
GET /api/v1/client-versions/current?client_key=android
...
`/api/v1/skills` 返回技能分类（第一期仅 `knots`）；绳结列表和详情读取数据库中的 Knots3D metadata，不暴露 Markdown mock。`/api/v1/content-pages/:page_key` 返回数据库中的客户端文案页，当前用于微信端和 Android 端的 `profile_about` 内容，响应包含 `eyebrow`、`title`、`subtitle`、`sections[]`、`button_text` 和 `updated_at`；缺少对应已发布行返回 `404`，不支持的 `client_key` 或 `locale` 返回 `422 validation_failed`。`/api/v1/client-versions` 和 `/api/v1/client-versions/current` 返回客户端版本说明；Android 当前由迁移强制维护为唯一公开版本 `0.0.1`，文案覆盖账号与我的页面、装备库、装备图鉴、户外技能/绳结、行程规划、轨迹导入/轨迹库、地图预览、离线缓存，以及关于页和版本信息读取数据库。`/api/v1/gear-templates` 和 `/api/v1/gear-templates/:id` 从数据库读取装备模板分类和条目；服务启动时会幂等写入默认系统模板，替代旧的 `content/gear-templates/*.yaml` 文件源。`/api/v1/gear-atlas` 和 `/api/v1/gear-atlas/:id` 返回已审核通过且 `is_deleted=false` 的公共装备图鉴，不包含用户个人购买、位置、标签、备注、拒绝原因、原始投稿快照、审核改动摘要、来源名称、来源链接或来源评分字段；响应保留 `created_at`、`updated_at` 和 `is_deleted` 供客户端统一显示记录时间与可见性状态。图鉴公共尺寸使用 `variants` 数组表示，每项包含 `key`、`label`，以及可选 `official_price_cents`、`official_price_currency`、`weight_g`；分类参数 `specs` 不再接受或返回 `size`、`backpack_size`、`size_or_length`。外部导入来源只在管理员审核接口暴露 `source_name`、`source_url`、`source_rating_score` 和 `source_rating_count` 等审计摘要，不暴露内部去重键、导入批次或授权备注；管理员响应另返回只读 `display_name`、`display_description`、`display_variants`、`display_specs` 和 `display_category_label`，用于按请求语言展示，不覆盖可编辑 canonical 字段。

用户自己的 `GET /api/v1/me/gear-atlas-submissions` 和管理员审核接口会返回投稿状态字段 `status`、可选 `rejection_reason`、以及审核通过时的 `review_changes`。`review_changes` 是数组，每项包含 `field`、中文 `label`、`before` 和 `after`，表示管理员按原始投稿快照和最终通过值生成的公共字段差异。管理员列表默认只返回 `is_deleted=false`，可用 `deleted=active|deleted|all` 切换可见性。管理员审核接口也解析 `X-StellarTrail-Locale` / `Accept-Language`，按请求语言填充 `review_locale` 和 `display_*` 字段；`name`、`description`、`variants` 和 `specs` 仍是待审核 canonical 字段。列表和详情都会返回 `localization_statuses`，详情额外返回 `localizations`，用于审核 `zh-CN` 和 `en` 两种展示内容。管理员 `PUT /api/v1/admin/gear-atlas-submissions/:id/localizations/:locale` 保存单语言展示内容，`mark_reviewed=true` 时把该语言标记为已审核；`POST /api/v1/admin/gear-atlas-submissions/:id/localizations/:locale/generate-draft` 手动生成单语言草稿，默认不覆盖已审核内容。管理员 `PATCH /api/v1/admin/gear-atlas-submissions/:id` 只能替换图鉴公共字段；`POST /api/v1/admin/gear-atlas-submissions/:id/approve` 会要求 `zh-CN` 和 `en` localization 均存在、必填字段完整且 `translation_status=reviewed`，否则返回 `422 validation_failed` 和具体缺失项；`DELETE /api/v1/admin/gear-atlas-submissions/:id` 会设置 `is_deleted=true`，`POST /api/v1/admin/gear-atlas-submissions/:id/restore` 会恢复；`POST /api/v1/admin/gear-atlas-submissions/:id/reject` 必须提交非空 `reason`，空白原因返回 `422`。
...
装备模板的模板标题、分类名、条目名存储在 `gear_template_*_localizations` 表中；默认系统模板同时 seed `zh-CN` 和 `en`。装备图鉴的公共 `name`、`description`、`variants` 和 `specs` 可存储在 `gear_atlas_item_localizations` 表中；新用户投稿默认写入 `zh-CN` 原文行，不做自动机翻；外部导入会为来源语言和目标语言各写一行 localization。公共 `category_label` 从 `gear_category_localizations` 表解析。`brand`、`model`、数字、单位、价格、重量和来源审计字段不做翻译。
...
外部装备来源记录会在 `gear_atlas_items` 保留 `source_key`、`source_name`、`source_url`、`source_rating_score` 和 `source_rating_count` 等审核溯源字段，并在内部 `gear_atlas_import_sources` 表记录 `canonical_key`、来源 locale、detail score、last action 和最近 batch。公开图鉴接口不暴露这些字段；只有管理员审核接口返回来源摘要，且不会返回内部去重键、导入批次或授权备注。

多来源导入工具位于 `crates/importer`，二进制名为 `import-gear-atlas-all`，支持 `probe`、`discover`、`dry-run`、`write` 和 `backfill-localizations`。`discover` 可用 `--output-discovery-file` 写 JSONL，`dry-run` 和 `write` 可用 `--from-discovery-file` 消费该文件；`--max-items-per-source` 接受数字或 `unlimited`；长任务可用 `--resume-state-file` 和 `--source-url-cache-file` 断点续跑和去重。`write` 必须同时满足 `--write --database-url --submitter-user-id --batch-id --translation-provider`，并设置 `STELLARTRAIL_IMPORT_ENV=test`，用于测试环境批量写入待审核图鉴。`backfill-localizations` 同样要求 `--write --database-url --translation-provider` 和 `STELLARTRAIL_IMPORT_ENV=test`，只补英文来源 pending 条目的 `zh-CN` localization，不修改主表 `name`、来源链接或审核字段。当前来源策略是：8264 只有在站点负责人授权场景下允许 `discover --full-scan --ignore-robots 8264`，并使用受控 ID 扫描和断点记录；默认 8264 起点锚定在已验证的装备 ID 区间附近，可通过 `--8264-scan-start`、`--8264-scan-end` 和 `--8264-consecutive-miss-limit` 扩大或缩小扫描范围；PackWizard 通过 sitemap 发现英文详情；GearAtlas 递归 WP sitemap；GearKr 分页读取 WP REST；Trailspace 和 OutdoorGearReview 只抓允许的公开结构化事实字段，其中 OutdoorGearReview 强制 30 秒以上延迟；REI、PackBase 和 retailer/affiliate 受限内容不自动抓取。导入器不保存第三方图片、用户点评、长评测正文或联盟链接。
