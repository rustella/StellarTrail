...
本地环境响应会带 `debug_code` 方便联调；生产环境不返回明文验证码，服务端会通过配置的 SMTP 邮箱发送验证码。当前生产发件邮箱为 `noreply@stellartrail.cn`，SMTP 主机为 `mx1.stellartrail.cn`，真实密码只放在 `.env`、被忽略的 `config.yaml` 或 secret manager：
