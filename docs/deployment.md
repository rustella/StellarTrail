...
请求签名只从被忽略的 YAML 配置文件读取，不支持环境变量覆盖。生产启用时在真实 `config.yaml` 或 `config.*.yaml` 中配置 `request_signature.enabled=true`，并把真实 `request_signature.clients[].app_id` / `app_secret` 留在该未跟踪文件内；仓库示例文件只能保留占位值。
