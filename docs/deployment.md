...
Web 可通过 `VITE_STELLARTRAIL_API_BASE_URL` 和 `VITE_STELLARTRAIL_ASSETS_BASE_URL` 覆盖；本地 Vite 开发默认使用同源 `/api/v1`，并通过 `VITE_STELLARTRAIL_API_PROXY_TARGET` 代理到真实或本地 API，避免浏览器 CORS 拦截。启用后端请求签名时，Web 构建还必须设置 `VITE_STELLARTRAIL_REQUEST_SIGNATURE_APP_ID` 和 `VITE_STELLARTRAIL_REQUEST_SIGNATURE_APP_SECRET`，并确保同一组 `app_id/app_secret` 已登记到目标 API 的 ignored YAML 配置。微信小程序端会读取 `miniprogram/config.ts`，缺失时回退到占位地址。
...
请求签名只从被忽略的 YAML 配置文件读取，不支持环境变量覆盖。测试和生产启用时分别在目标环境真实 `config.yaml` 或 `config.*.yaml` 中配置 `request_signature.enabled=true`，并把真实 `request_signature.clients[].app_id` / `app_secret` 留在该未跟踪文件内；仓库示例文件只能保留占位值。Web、微信小程序、Android 等客户端应使用各自独立的签名客户端；测试 Web 默认使用类似 `stellartrail-test-web` 的 `app_id`，生产 Web 默认使用类似 `stellartrail-prod-web` 的 `app_id`。浏览器构建中的 Web `app_secret` 是公开客户端凭据，不能替代服务端私密鉴权。

测试机 persistent compose 若使用 `infra/test/docker-compose.testhost.yml`，应在该 server-local ignored 文件中把包含 `request_signature` 的 API YAML 只读挂载到容器，并设置 `CONFIG_PATH` 指向它。例如：
...
短信验证码生产投递通过阿里云号码认证服务。启用时把 `sms.enabled`、`sms.access_key_id`、`sms.access_key_secret` 等写入被忽略的 `config.yaml`，如需指定方案再填写 `sms.scheme_name`；也可以由 secret manager 渲染为同等 YAML 文件后挂载到 `/app/config.yaml`。手机号发送频控通过 `sms.phone_rate_limit` 配置，同一规范化手机号默认 60 秒内最多 1 次、滚动 24 小时内最多 20 次。
...
API 容器通过 Docker 服务名访问 `postgres`、`redis`、`minio`，并把服务器根目录的真实 `config.yaml` 只读挂载到 `/app/config.yaml`。生产 API 必须通过 `infra/production/api/compose-from-config.sh` 部署，不要在生产服务器裸跑 `docker compose -f docker-compose.yml up`。该脚本会从 YAML 派生 PostgreSQL、Redis、MinIO 的 Compose 运行变量，并在服务器本地存在 `docker-compose.production-local.override.yml` 时自动加载它；该 ignored override 用于绑定既有生产卷、Traefik entrypoint / resolver 或其他服务器本地 Compose 差异，避免部署时误建新的 PostgreSQL、Redis 或 MinIO 数据卷。

生产已有数据后，服务器本地 `docker-compose.production-local.override.yml` 必须把三个状态卷都声明为 external 并写明实际卷名。`compose-from-config.sh` 会在 `up`、`create`、`start`、`run` 前校验这些卷绑定；只有第一次全新初始化生产环境时，才可以显式设置 `STELLARTAIL_ALLOW_UNPINNED_PRODUCTION_VOLUMES=1` 跳过。
...
volumes:
  ...
  redis-data:
    external: true
    name: <existing-redis-volume>
  minio-data:
    external: true
    name: <existing-minio-volume>
```

生产 API 不使用 `infra/production/api/.env`。仓库只提交 `config.example.yaml`、`*.example.yaml` 和非敏感 Compose 模板；真实配置、证书存储、密钥文件和服务器本地 `*.override.yml` 必须由部署环境注入并保持 ignored。
