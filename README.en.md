...
- `infra/production/api/docker-compose.yml`: API service plus private PostgreSQL, Redis, and MinIO dependencies; the API reaches them through Docker service names `postgres`, `redis`, and `minio`, root `config.yaml` is mounted to `/app/config.yaml:ro`, and `assets.stellartail.cn` reaches only the MinIO API through Traefik while the MinIO console stays private.
