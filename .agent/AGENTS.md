...
8. Migration files under `crates/migration/src/` are initialization schema units, not an append-only production migration history. Fold new schema columns, tables, and indexes into the relevant existing initialization migration instead of adding standalone schema patch migrations. Preserve existing compatibility no-op markers only when they are already needed by recorded local migration history, and keep data-repair migrations separate only when they intentionally transform seeded or imported data.
