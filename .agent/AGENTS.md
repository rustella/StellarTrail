...
8. Migration files under `crates/migration/src/` are initialization schema units, not an append-only production migration history. Fold new schema columns, tables, indexes, seed rows, copy changes, and product-data fixes into the relevant existing `create_*` migration instead of keeping temporary standalone `add_*`, `ensure_*`, or `update_*` migration files. If a migration name has already been deployed or recorded in local databases, keep that history only as a no-op marker in `compat_folded_migrations.rs`. A PR that introduces temporary migration code must fold it back into the main migration or add the compat marker before handoff, unless the migration is an intentional long-lived data-safety cleanup such as imported public-content sanitization.
